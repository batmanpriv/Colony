...
struct resolv_entries *resolv_lookup(char *domain)
{
    ...
    while (tries++ < 5)
    {
        ...
        {
#ifdef DEBUG
            printf("dbg: Failed to create socket\n");
#endif
            sleep(1);
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("dbg: Failed to call connect on udp socket\n");
#endif
            sleep(1);
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("dbg: Failed to send packet: %d\n", errno);
#endif
            sleep(1);
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("dbg: Couldn't resolve %s in time. %d tr%s\n", domain, tries, tries == 1 ? "y" : "ies");
#endif
            continue;
        }
        ...
        {
#ifdef DEBUG
            printf("dbg: Got response from select\n");
#endif
...
#ifdef DEBUG
                        printf("dbg: Found IP address: %08x\n", (*p));
#endif
                    }
...
#ifdef DEBUG
    printf("dbg: Resolved %s to %d IPv4 addresses\n", domain, entries->addrs_len);
#endif
